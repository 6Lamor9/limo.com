<script>import { onMount } from 'svelte';
    ...
    export let btnClass = 'text-2xl fixed bottom-4 left-4 z-10 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 rounded-lg text-sm p-2.5';
    export let size = 'md';
    const sizes = {
        sm: 'w-4 h-4',
        md: 'w-5 h-5',
    ...
    onMount(() => {
        ...
        toggleTheme = () => {
            ...
            //  data-theme
            const isDarkTheme = window.document.documentElement.classList.contains('dark');
            window.document.documentElement.setAttribute('data-theme', isDarkTheme ? 'night' : 'light');
